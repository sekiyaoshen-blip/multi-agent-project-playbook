...
On official OpenAI service, use only `gpt-5.6-luna` for `fast`,
`gpt-5.6-terra` for `balanced`, and `gpt-5.6-sol` for `deep`/`critical`. Owner returns that require
...
GPT-5.5 and older models are never used. The only OpenAI compatibility fallback
is `gpt-5.6-luna` with `medium`, used once before output when safe. Non-OpenAI
services receive no model/Thinking overrides and keep provider defaults. Native
system/Desktop delivery is preferred for returns; file-based Return Packets
