...
- 每次调用已有或获得授权后新建显性任务前，都先判断具体任务并显式传入当前工具支持的 `model` 和 `thinking`。
...
每次调用已有显性任务，包括模块之间再次转派，或者在获得授权后新建显性任务前，都判断任务类型、复杂度、风险、上下文、可逆性和并行性，并检查当前工具支持的参数，然后同时显式传入 `model` 和 `thinking`。
